...
// CORS configuration — open for LAN testing with physical devices
app.use(
  cors({
    origin: true,
    credentials: true,
...
wss.on('connection', (ws, req) => {
  ...
  ws.send(
    JSON.stringify({
      type: 'connection',
      message: 'Connected to HearClear server',
      timestamp: new Date().toISOString()
    })
  );
...
async function startServer() {
  try {
    ...
    httpServer.listen(PORT, '0.0.0.0', () => {
      console.log(`\n╔════════════════════════════════════════════════════════╗`);
      console.log(`║  HearClear Backend Server Started                      ║`);
      console.log(`╠════════════════════════════════════════════════════════╣`);
      console.log(`║  Environment: ${NODE_ENV.padEnd(43)}║`);
      console.log(`║  HTTP Server: http://0.0.0.0:${PORT.toString().padEnd(39)}║`);
      console.log(`║  WebSocket: ws://0.0.0.0:${PORT.toString().padEnd(39)}║`);
